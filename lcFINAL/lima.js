...
// Depok, Jakarta, Bandung, Surabaya, Bali


function cekKota(str){
...
function jarakKirim(asal, tujuan){
  var kota = ['Depok', 'Jakarta', 'Bandung', 'Surabaya', 'Bali']
  var asal1 = kota.indexOf(asal)
  var tujuan1 = kota.indexOf(tujuan)
  return asal1 == tujuan1 ? 1 : Math.abs(asal1-tujuan1);
}

function ongkosKirim(pengirim) {
  var output = [];
  for(var i = 0; i < pengirim.length; i++){
    if(cekKota(pengirim[i].asal) && cekKota(pengirim[i].tujuan)){
      var jarak = jarakKirim(pengirim[i].asal, pengirim[i].tujuan)
    }
    else{
      var jarak = 0;
    }
    output.push({
      ...
      tagihan :  pengirim[i].ekspedisi == 'NJE' ? jarak * 9000 - ((jarak * 9000)*0.1): jarak * 9000,
      durasi : jarak
    })
  }
  return output;
