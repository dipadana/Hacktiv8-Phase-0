...
//             if(j === 0 && k === 0){
//               result += '| ' + month[i] + ' | ' + ' Branch- ' + (j+1) + ' IN: ' + branches[j][0][i] + ' | OUT: ' + branches[j][1][i] + ' | ';
//               break;
//             }else if(j === branches.length-1){
...
function printBranch(branches) {
  ...
  for(var i = 0; i < month.length; i++){
    for(var j = 0; j < branches.length; j++){
      for(var k = 0; k < branches[j][0].length; k++){
        if(i === k && j === 0){
          result += '| ' + month[i] + ' |  Branch- ' + (j+1) +  ' IN: ' + branches[j][0][k] + ' | OUT: ' + branches[j][1][k]
        }else if(i === k && j > 0){
          result += ' |  Branch- ' + (j+1) +  ' IN: ' + branches[j][0][k] + ' | OUT: ' + branches[j][1][k]
        }
      }
    }
    result += '\n'
